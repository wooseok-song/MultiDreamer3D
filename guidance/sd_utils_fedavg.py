...
class StableDiffusionFedAVG(nn.Module):
    def __init__(
        self,
        device,
        fp16,
        ...
        if not self.ism:
            print("SDS WITH GUiDANCE: {}".format(guidance_opt.guidance_scale))
    ...
    def add_noise_with_cfg(
        ...
        cross_attention_kwargs = {"scale": self.cross_attn_scale}
        pred_scores = []
        for i in range(inv_steps):
            ...
            if cfg > 1.0:
                ...
                unet_output = unet(
                    latent_model_input, timestep_model_input, encoder_hidden_states=text_embeddings
                ).sample
