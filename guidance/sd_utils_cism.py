...
class StableDiffusionCISM(nn.Module):
    def __init__(
        self,
        device,
        fp16,
        ...
        self.concept_num = concept_opt.concept_num
        ...
        if is_safe_tensor:
            pipe = StableDiffusionPipeline.from_single_file(
                model_key, use_safetensors=True, torch_dtype=self.precision_t, load_safety_checker=False
            )
        else:
        ...
        for i in range(self.concept_num):
            ...
            pipe.load_lora_weights(concept_dict[concept_key]["step2_lora_path"], adapter_name=concept_key)
        pipe.set_adapters(cur_loras)
        print("Concept lora weight loaded")

        self.lora_adapters = pipe.get_active_adapters()
        self.cross_attn_scale = concept_opt.step2_lora_scale

        self.ism = not guidance_opt.sds
        self.scheduler = DDIMScheduler.from_pretrained(
            model_key if not is_safe_tensor else base_model_key, subfolder="scheduler", torch_dtype=self.precision_t
        )
    ...
    # Concept-aware Inversion
    def add_noise_with_cfg_cism(
        self,
        latents,
        concept_mask_dict,
        noise,
        ...
        inverse_text_embeddings_dict["bg"] = text_embeddings

        for i in range(self.concept_num):
            concept_key = f"concept{i}"
            inverse_text_embeddings_dict[concept_key] = text_embeddings
        ...
        for i in range(inv_steps):
            ...
            if cfg > 1.0:
                ...
                timestep_model_input = (
                    self.timesteps[cur_ind_t].reshape(1, 1).repeat(cur_noisy_lat_.shape[0], 1).reshape(-1)
                )
                unet_output = unet(
                    cur_noisy_lat_,
                    timestep_model_input,
    ...
    def train_step_cism(
        ...
        # Augmentation
        pred_rgb, pred_concept_mask, pred_depth, pred_alpha = self.augmentation(
            pred_rgb, pred_concept_mask, pred_depth, pred_alpha
        )

        # Text embeddings without concept token
        text_embeddings = text_embeddings_dict["base"]

        # Batch size
        B = pred_rgb.shape[0]

        # Cond text embedding size
        K = text_embeddings.shape[0] - 1

        # Randomly select latent or image space
        if as_latent:
        ...
        for key in text_embeddings_dict.keys():
            text_embeddings_dict[key] = text_embeddings_dict[key].reshape(
                -1, text_embeddings.shape[-2], text_embeddings.shape[-1]
            )
        ...
        # Preprocess Concept Masks for CISM Inversion
        concept_mask_dict = {}

        latents_concept_mask = F.interpolate(pred_concept_mask, (64, 64), mode="nearest")
        latents_concept_mask = torch.cat([latents_concept_mask], dim=0)
        bg_mask = latents_concept_mask[:, : self.concept_num, :, :]
        bg_mask = (~torch.any(bg_mask == 1, dim=1, keepdim=True)).float()

        concept_mask_dict["bg"] = bg_mask

        for i in range(
            self.concept_num,
        ):
            concept_mask_dict[f"concept{i}"] = latents_concept_mask[:, i : i + 1, :, :]
        ...
        with torch.no_grad():
            ...
            if not self.ism:
                ...
                _, prev_latents_noisy, pred_scores_xs = self.add_noise_with_cfg_cism(
                    latents,
                    concept_mask_dict,
                    noise,
                    ind_prev_t,
                ...
                # Step 2: sample x_t
                _, latents_noisy, pred_scores_xt = self.add_noise_with_cfg_cism(
                    prev_latents_noisy,
                    concept_mask_dict,
                    noise,
                    ind_t,
        ...
        # Preprocess Cocnept Masks for CISM Inversion
        concept_mask_dict = {}

        latents_concept_mask = F.interpolate(pred_concept_mask, (64, 64), mode="nearest")
        latents_concept_mask = torch.cat([latents_concept_mask] * 2, dim=0)
        bg_mask = latents_concept_mask[:, : self.concept_num, :, :]
        bg_mask = (~torch.any(bg_mask == 1, dim=1, keepdim=True)).float()
        ...
        with torch.no_grad():
            ...
            if use_control_net:
                pred_depth_input = (
                    pred_depth_input[None, :, ...].repeat(1 + K, 1, 3, 1, 1).reshape(-1, 3, 512, 512).half()
                )
                ...
                cross_attention_kwargs = {
                    "mask": concept_mask_dict,
                    "adapter_names": self.lora_adapters,
                    "text_embeddings": text_embeddings_dict,
                    "lora_scale": self.cross_attn_scale,
                    "RCA": True,
                }
                unet_output = self.unet(
        ...
        if iteration % guidance_opt.vis_interval == 0:
            ...
            lat2rgb = lambda x: torch.clip(
                (x.permute(0, 2, 3, 1) @ self.rgb_latent_factors.to(x.dtype)).permute(0, 3, 1, 2), 0.0, 1.0
            )
            ...
            with torch.no_grad():
                ...
                latents_rgb = F.interpolate(
                    lat2rgb(latents), (resolution[0], resolution[1]), mode="bilinear", align_corners=False
                )
                latents_sp_rgb = F.interpolate(
                    lat2rgb(pred_x0_latent_sp), (resolution[0], resolution[1]), mode="bilinear", align_corners=False
                )
