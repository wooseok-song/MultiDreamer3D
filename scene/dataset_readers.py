...
from utils.shape_utils import init_from_shape, init_from_shape_candidates
...
class RSceneInfoConcept(NamedTuple):
    test_cameras: list
...
# only test_camera
def readCircleCamInfoConcept(path, opt, concept_opts, concept_dict):
    print("Reading Test Transforms")
    test_cam_infos = GenerateCircleCameras(opt, render45=opt.render_45)
    # Generating pointlcoud for each concepts
    if concept_opts.step1_layout:
        opt.init_shape = "shape"
    else:
        opt.init_shape = "sphere"

    for i in range(concept_opts.concept_num):
        concept_key = f"concept{i}"
        ply_path = os.path.join(path, f"init_points3d_concept{i}.ply")
        ply_rgb_path = os.path.join(path, f"init_points3d_concept{i}_rgb.ply")

        if concept_opts.step1_load_from_pcd_path:
            print(f"Loading pcd concept{i} from {getattr(concept_opts, f'step1_concept{i}_pcd_path')}")
            pcd_path = getattr(concept_opts, f"step1_concept{i}_pcd_path")
            pcd = fetchPly(pcd_path)
            storePly(ply_path, pcd.points, pcd.colors * 255)
            concept_dict[concept_key]["step1_pcd"] = pcd
            concept_dict[concept_key]["step1_pcd_rgb"] = None
            concept_dict[concept_key]["step1_pcd_path"] = ply_path
            continue
        ...
        if not os.path.exists(ply_path):
            ...
            if opt.init_shape == "sphere":
                ...
                pointe_prompt = getattr(opt, f"init_concept{i}_prompt")
                print(f"Pointe_prompt: {pointe_prompt}")
                xyz, rgb = init_from_pointe(pointe_prompt)
                xyz[:, 1] = -xyz[:, 1]
                xyz[:, 2] = xyz[:, 2] + 0.15
                thetas = np.random.rand(num_pts) * np.pi
                phis = np.random.rand(num_pts) * 2 * np.pi
                ...
                xyz = xyz * 1.0
                num_pts = xyz.shape[0]
            elif opt.init_shape == "shape" and concept_opts.step1_layout:
                num_pts = int(num_pts / 5000)
                shape_prompt = concept_dict[concept_key]["step1_prompt"]

                if concept_opts.step1_point_cloud_selection:
                    print(
                        f"=================================== Point Cloud Selection for concept {i} ==================================="
                    )
                    print(f"Shape_prompt: {shape_prompt}")
                    shape_cand_path = os.path.join(path, f"shape_candidates/concept{i}")
                    if not os.path.exists(shape_cand_path):
                        os.makedirs(shape_cand_path)

                    xyz, rgb = init_from_shape_candidates(concept_opts, shape_prompt, shape_cand_path)
                else:
                    print(f"Shape_prompt: {shape_prompt}")
                    xyz, rgb = init_from_shape(shape_prompt)
            ...
            print(f"Point cloud generated ({num_pts})...")

            shs = np.random.random((num_pts, 3)) / 255.0

            # NOTE First save pcd for refinement module
            if opt.init_shape == "shape":
                pcd_rgb = BasicPointCloud(points=xyz, colors=rgb, normals=np.zeros((num_pts, 3)))
                storePly(ply_rgb_path, xyz, rgb * 255)
                rgb_pcd = fetchPly(ply_rgb_path)
            else:
                pcd_rgb = BasicPointCloud(points=xyz, colors=SH2RGB(shs), normals=np.zeros((num_pts, 3)))
                storePly(ply_rgb_path, xyz, SH2RGB(shs) * 255)
                rgb_pcd = fetchPly(ply_rgb_path)
            ...
            elif opt.init_shape == "shape" and opt.use_pointe_rgb:
                pcd = BasicPointCloud(points=xyz, colors=rgb, normals=np.zeros((num_pts, 3)))
                storePly(ply_path, xyz, rgb * 255)
            else:
        ...
        concept_dict[concept_key]["step1_pcd"] = pcd
        concept_dict[concept_key]["step1_pcd_rgb"] = rgb_pcd
        concept_dict[concept_key]["step1_pcd_path"] = ply_path

    scene_info = RSceneInfoConcept(test_cameras=test_cam_infos)
    return scene_info
...
def circle_poses(
    radius=torch.tensor([3.2]), theta=torch.tensor([60]), phi=torch.tensor([0]), angle_overhead=30, angle_front=60
):
...
def gen_random_pos(size, param_range, gamma=1):
    ...
    sign = torch.where(torch.rand(size) > 0.5, torch.ones(size) * -1.0, torch.ones(size))
...
def GenerateCircleCameras(opt, size=8, render45=False):
    ...
    for idx in range(size):
        ...
        poses = circle_poses(
            radius=radius, theta=thetas, phi=phis, angle_overhead=opt.angle_overhead, angle_front=opt.angle_front
        )
    ...
    for idx in range(size):
        ...
        cam_infos.append(
            RandCameraInfo(
                ...
                width=image_w,
                height=image_h,
                delta_polar=delta_polar[idx],
                delta_azimuth=delta_azimuth[idx],
...
def GeneratePurnCameras(opt, size=300):
    ...
    poses, thetas, phis, radius = rand_poses(
        size,
        opt,
        radius_range=[opt.default_radius, opt.default_radius + 0.1],
        theta_range=opt.theta_range,
        phi_range=opt.phi_range,
        angle_overhead=opt.angle_overhead,
        angle_front=opt.angle_front,
...
def GenerateRefineCameras(opt, size=4, render45=False):

    # random focal
    fov = opt.refine_default_fovy
    cam_infos = []
    # generate specific data structure

    for idx in range(size):
        thetas = torch.FloatTensor([opt.refine_default_polar * 2 // 3])
        phis = torch.FloatTensor([(idx / size) * 360])
        radius = torch.FloatTensor([opt.refine_default_radius])
        # random pose on the fly
        poses = circle_poses(
            radius=radius,
            theta=thetas,
            phi=phis,
            angle_overhead=opt.refine_angle_overhead,
            angle_front=opt.refine_angle_front,
        )
        matrix = np.linalg.inv(poses[0])
        R = -np.transpose(matrix[:3, :3])
        R[:, 0] = -R[:, 0]
        T = -matrix[:3, 3]
        fovy = focal2fov(fov2focal(fov, opt.refine_image_h), opt.refine_image_w)
        FovY = fovy
        FovX = fov

        # delta polar/azimuth/radius to default view
        delta_polar = thetas - opt.refine_default_polar
        delta_azimuth = phis - opt.refine_default_azimuth
        delta_azimuth[delta_azimuth > 180] -= 360  # range in [-180, 180]
        delta_radius = radius - opt.refine_default_radius
        cam_infos.append(
            RandCameraInfo(
                uid=idx + size,
                R=R,
                ...
                FovY=FovY,
                FovX=FovX,
                width=opt.refine_image_w,
                height=opt.refine_image_h,
                delta_polar=delta_polar,
                delta_azimuth=delta_azimuth,
                delta_radius=delta_radius,
            )
    ...
    for idx in range(size):
        thetas = torch.FloatTensor([opt.refine_default_polar * 1 // 3])
        phis = torch.FloatTensor([(idx / size) * 360])
        radius = torch.FloatTensor([opt.refine_default_radius])
        # random pose on the fly
        poses = circle_poses(
...
def GenerateCircleCamerasPolars(opt, size=8, polars=60):

    # random focal
    fov = opt.default_fovy
    cam_infos = []
    for idx in range(size):
        thetas = torch.FloatTensor([polars])
        phis = torch.FloatTensor([(idx / size) * 360])
        radius = torch.FloatTensor([opt.default_radius])
        # random pose on the fly
        poses = circle_poses(
...
sceneLoadTypeCallbacks = {
    ...
    "RandomCamConcept": readCircleCamInfoConcept,
}
